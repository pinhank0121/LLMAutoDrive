...
example_answer = textwrap.dedent(
    """\
        ...
        - Besides decelearation, I can also try to change lanes. I should carefully check the distance and speed of the c/tmp2/pinhank/.cache/huggingface/hub/models--TheBloke--Llama-2-7b-Chat-GGUF/ars in front of me on the left and right lanes. Noted that change-lane is not a frequent action, so I should not change lanes too frequently.
...
class DriverAgent:
    def __init__(
        self, sce: EnvScenario, temperature: float = 0, verbose: bool = False
    ) -> None:
        self.sce = sce
        ...
        if oai_api_type == "azure":
            ...
            # NOTE: server_url = 'http://localhost:8000/v1'
            server_url = os.getenv("OPENAI_SELF_HOST_SOURCE")
            print(f"Use Local Server {server_url}")
            self.llm = ChatOpenAI(
    ...
    def few_shot_decision(
        ...
        try:
            ...
            # print(f"Output {decision_action=} is not a int number, checking the output...")
            print("Output is not a int number, checking the output...")
            check_message = f"""
            You are a output checking assistant who is responsible for checking the output Action_id of another agent.
            ...
            Only answer the Action_id number within 0-4, do not output any other content.
            Response to user in the format:
            {delimiter}<Action_id within 0-4>
            """
            messages = [
            ...
            try:
                num = check_response.content.split(delimiter)[-1].strip()
                num = num.split(" ")[0]
                result = int(num)
            except ValueError:
                print("Invalid Response:", check_response.content)
                print(
                    "Invalid Response parsed:",
                    check_response.content.split(delimiter)[-1],
                )
            # TODO: Still have the possibility to get invalid action decode result.
            # Maybe should default to IDLE action if cannot decode.
